                    ...
                    {filter.length !== 0 &&
                      <Accordion summary={
                        ...
                        <div className='flex flex-wrap gap-1 pb-4'>
                          {filter.map((item, idx) => (<FilterBadge key={idx} remove={() => handleRemove(item)}>{item.value}</FilterBadge>))}
                        </div>
                      </Accordion>}
                  </div>
                  {
                ...
                {filter.length !== 0 &&
                  <div className="mt-4 border-y py-3 border-app-gray-20">
                    <div className='md:flex flex-wrap gap-1 hidden'>
                      {filter.map((item, idx) => (<FilterBadge key={idx} remove={() => handleRemove(item)} mode="purple">{item.value}</FilterBadge>))}
